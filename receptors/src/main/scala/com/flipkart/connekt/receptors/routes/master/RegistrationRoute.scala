                        ...
                        meteredResource(s"patch.$platform.$appName") {
                          entity(as[Map[String, AnyRef]]) { patchedDevice =>
                            if (!isTestRequest) {
                              ...
                              result match {
                                case Right(_) =>
                                  complete(GenericResponse(StatusCodes.OK.intValue, null, Response(s"No DeviceDetails exists for $deviceId returning 200", null))) //returning 200 for honouring varadhi contract
                                case Left(d) =>
